...
- **La lueur d'aube** (l'histoire de la page) : une lueur rosée très subtile en tête de page, présente dans les deux thèmes — la page semble imprimée au lever du jour. Statique, aucune logique horaire ; c'est un dégradé token (`--gradient-dawn`) dans la couche primitive, appliqué au shell. Discrète au point de ne jamais gêner la lecture (contraste vérifié lueur comprise).

...
import { buildShareCardJSX } from './ShareCardRenderer';
...
export function ShareButton({ artUrl, title, artist, trackUrl }: ShareButtonProps) {
  ...
  const handleShare = useCallback(() => {
    ...
    void (async () => {
      try {
        // satori serializes the JSX directly — no off-screen DOM mount needed.
        const blob = await generateShareImage(buildShareCardJSX({ artUrl, title, artist }));
